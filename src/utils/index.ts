...
export const base32ToHex = (s: string, alphabet = RFC4648_ALPHABET) =>
  Array.from(new Uint8Array(_base32Decode(s, alphabet)))
    .map((x) => x.toString(16))
    .join("");
...
function decode(buf: ArrayBuffer, codeType: CODE_TYPE = "40") {
  ...
  const { info } = bencode.decode(buffer as Buffer);
...
export async function torrentToMagnet(torrent: string): Promise<string> {
  const buffer = await fetch(torrent).then((res) => res.arrayBuffer());
